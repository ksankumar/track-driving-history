<template>
  <v-layout align-center justify-center row fill-height>
    ...
    <v-flex xs12 sm10 offset-sm1 v-else>
      <v-card width="100%">
            ...
            <v-card flat width="100%" class="content-scroll" max-height="400px">
              <v-list two-line class="py-0">
                <transition-group name="list">
                  <div v-for="(item, index) in computedDrivers" v-bind:key="index">
                    <v-list-tile avatar ripple @click.stop="selectedItem=item">
                      <v-list-tile-avatar>
          ...
          <v-flex xs6 text-xs-center>
            <v-card flat height="450px">
              <v-slide-x-reverse-transition mode="out-in">
                ...
                <v-layout v-else align-center fill-height :key="selectedItem.id">
                  <v-card class="mx-auto" flat width="80%">
                    ...
                    <v-card-text class="py-0">
                      <v-avatar color="app" size="88">
                        <span class="white--text text-uppercase" style="font-size: 64px"
                    ...
                    <v-card-text>
                      ...
                      <v-layout row wrap>
                        <v-flex xs6><strong>Issued on : </strong>{{ selectedItem.licenceIssueDate }}</v-flex>
                        <v-flex xs6 pl-2><strong>Expires :</strong>{{ selectedItem.licenceExpiresOn }}</v-flex>
                      </v-layout>
                    </v-card-text>
                    <v-divider></v-divider>
                    <v-flex xs12 pt-1 class="blue-grey--text font-weight-bold">Total Trip Summary</v-flex>
                    <v-card-text class="py-2" v-if="selectedItem.summary">
                      <v-layout row wrap>
                    ...
                    <v-card-actions>
                      <v-btn block color="primary" class="text-none"
                             :to="{name: 'CreateTrip', params: { driverId: selectedItem.id }}">Create Trip
                      </v-btn>
                    </v-card-actions>
                  </v-card>
                </v-layout>
...
<script>
  ...
  export default {
    ...
    computed: {
      drivers () {
        const list = this.$store.getters['Trip/DriverList']
        this.setDriverList(list)
        return list
      },
      computedDrivers () {
        const list = this.$store.getters['Trip/DriverList']
        ...
        return list.filter(item => {
          return item.first.toLowerCase().indexOf(search) !== -1 || item.last.toLowerCase().indexOf(search) !== -1 || item.email.toLowerCase().indexOf(search) !== -1
        })
      }
    },
    methods: {
